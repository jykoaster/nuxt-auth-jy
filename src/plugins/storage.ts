...
export class Storage {
  ...
  _initState(): void {
    ...
    if (this._useVuex) {
      ...
      this.ctx.store.registerModule('authCustom', storeModule, {
        preserveState: Boolean(this.ctx.store.state.authCustom),
      })

      this.state = this.ctx.store.state.authCustom
    } else {
      set(this, 'state', {})
  ...
  setState<V extends unknown>(key: string, value: V): V {
    if (key[0] === '_') {
      ...
      this.ctx.store.commit('authCustom/SET', {
        key,
        value,
      })
    } else {
